...
function FleetTable({ items, sources }: { items: CustomerLatest[]; sources: ArtifactSource[] }) { if (!items.length) return <Empty text="No authoritative deployment events have been received yet." />; return <div className="table-wrap fleet-table"><table><thead><tr><th>Customer name</th><th>BC version</th><th>Installed version</th><th>Installed date</th><th className="status-column">Status</th></tr></thead><tbody>{items.map(item => { const expected = sources.find(source => source.bcVersion === item.bcVersion); const current = Boolean(item.packageVersion && expected?.packageVersion === item.packageVersion && item.status === 'success'); const accessible = current ? 'Current' : item.status === 'success' ? 'Package differs from the selected artifact' : item.status === 'partial' ? 'Partial deployment' : item.status === 'failed' ? 'Deployment failed' : item.status; return <tr key={item.customerId}><td><Link to={`/customers/${item.customerId}`}><strong>{item.customerName}</strong></Link></td><td>{item.bcVersion ? `BC${item.bcVersion}` : 'Not reported'}</td><td><span className={`version-pill ${current ? 'current' : ''}`}>{item.packageVersion || 'Not reported'}</span></td><td><Time value={item.completedAt} /></td><td className="status-column"><Tooltip content={accessible} relationship="label">{current ? <CheckmarkCircleRegular className="status-icon current" aria-label={accessible} /> : <WarningRegular className="status-icon warning" aria-label={accessible} />}</Tooltip><span className="sr-only">{accessible}</span></td></tr> })}</tbody></table></div> }
