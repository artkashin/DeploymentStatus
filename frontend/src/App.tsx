...
function Baseline({ item }: { item: ArtifactSource }) { const label = item.usable ? item.warning ? 'Usable with warning' : 'Usable' : 'Unavailable'; return <div className="baseline"><span>BC{item.bcVersion} · {item.branch}</span><strong>{item.packageVersion || 'No package'}</strong><Badge appearance="filled" color={item.usable && !item.warning ? 'success' : item.usable ? 'warning' : 'danger'}>{label}</Badge><span className="baseline-completed">Completed <Time value={item.completedAt} /></span>{item.runUrl && <a href={item.runUrl} target="_blank" rel="noreferrer">View build <OpenRegular /></a>}</div> }
