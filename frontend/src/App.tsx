...
function ErrorState({ error }: { error: unknown }) { const message = error instanceof Error ? error.message : 'Unexpected error.'; const title = message.includes('session expired') ? 'Session expired' : message.includes('not authorized') ? 'Access not authorized' : 'Could not load deployment data'; return <main className="centered"><div className="error-state"><strong>{title}</strong><p>{message}</p></div></main> }
