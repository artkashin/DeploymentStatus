...
function FleetTable({ items }: { items: CustomerLatest[]; sources: ArtifactSource[] }) { if (!items.length) return <Empty text="No authoritative deployment events have been received yet." />; return <div className="table-wrap fleet-table"><table><thead><tr><th>Customer name</th><th>BC version</th><th>Desired apps</th><th>Last checked</th><th className="status-column">Status</th></tr></thead><tbody>{items.map(item => { const label = item.health === 'current' ? 'All desired apps current' : item.health === 'failed' ? `${item.failedAppCount} app update failed` : item.health === 'attention' ? `${item.attentionAppCount} app${item.attentionAppCount === 1 ? '' : 's'} need attention` : 'Desired apps not reported'; return <tr key={item.customerId} className={`fleet-${item.health}`}><td><Link to={`/customers/${item.customerId}`}><strong>{item.customerName}</strong></Link></td><td>{item.bcVersion ? `BC${item.bcVersion}` : 'Not reported'}</td><td><span className={`version-pill ${item.health === 'current' ? 'current' : item.health === 'failed' ? 'failed' : ''}`}>{item.desiredAppCount ? `${item.currentAppCount}/${item.desiredAppCount} current` : 'Not reported'}</span></td><td><Time value={item.completedAt} /></td><td className="status-column"><Tooltip content={label} relationship="label">{item.health === 'current' ? <CheckmarkCircleRegular className="status-icon current" aria-label={label} /> : <WarningRegular className={`status-icon ${item.health === 'failed' ? 'failed' : 'warning'}`} aria-label={label} />}</Tooltip><span className="sr-only">{label}</span></td></tr> })}</tbody></table></div> }
function DesiredAppTable({ grouped }: { grouped: Record<string, import('./types').DesiredAppState[]> }) { if (Object.keys(grouped).length === 0) return <Empty text="No desired-app inventory has been reported by an execute deployment yet." />; return <>{Object.entries(grouped).map(([tenant, states]) => <div className="tenant" key={tenant}><h3>{tenant}</h3><div className="table-wrap"><table className="desired-app-table"><thead><tr><th>Application</th><th>Desired version</th><th>Installed version</th><th>Last checked</th><th>Status</th></tr></thead><tbody>{states!.map(state => { const label = state.state === 'current' ? 'Current' : state.state === 'failed' ? 'Update failed' : state.state === 'outdated' ? 'Outdated' : state.state === 'planned' ? 'Planned' : 'Installed version unavailable'; return <tr key={`${state.tenantId}-${state.applicationId}`} className={`app-${state.state}`}><td><strong>{state.applicationName}</strong>{state.safeMessage && <small>{state.safeMessage}</small>}</td><td>{state.desiredVersion || 'Package unavailable'}</td><td>{state.installedVersion || 'Unavailable'}</td><td>{state.observedAt ? <Time value={state.observedAt} /> : '—'}</td><td>{label}</td></tr> })}</tbody></table></div></div>)}</> }
