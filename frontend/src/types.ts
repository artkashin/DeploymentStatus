...
export interface CustomerLatest { customerId: string; customerName: string; eventId: string; status: RunStatus; mode: DeploymentMode; completedAt: string; summary: Summary; bcVersion?: string; packageVersion?: string; customerType?: string; typeApps: string[]; desiredAppCount: number; currentAppCount: number; attentionAppCount: number; failedAppCount: number; health: 'current' | 'attention' | 'failed' | 'unknown'; tenants: TenantLatest[] }
